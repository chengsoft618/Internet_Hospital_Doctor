...
public final class EncryptUtils {
    ...
    public static String encryptRSA(@NonNull String content, InputStream publicKeyIS, String transformation) {
        byte[] data = content.getBytes();
        byte[] key = base64Decode(readRSAKeyString(publicKeyIS).getBytes());
        if (data == null || data.length == 0 || key == null || key.length == 0) {
            return null;
        }
        try {
    ...
    private static String readRSAKeyString(InputStream keyString) {
        BufferedReader br = new BufferedReader(new InputStreamReader(keyString));
