...
import cn.longmaster.hospital.doctor.core.requests.rounds.FinishOrderVisitRequester;
 ...
 * <p/>
 * 代码结构：
 * 复写的父类方法{@link RoundsConsultRoomActivity#onCreate(Bundle)}
 * <p/>
 * 初始化{@link RoundsConsultRoomActivity#initData()}
 * <p/>
 * 视图点击事件监听{@link RoundsConsultRoomActivity#onClick(View)}
 * <p/>
 * 房间状态操作相关{@link RoundsConsultRoomActivity#setPccParam()}
 * <p/>
 * 视频事件回调{@link RoundsConsultRoomActivity.InnerLMVideoEvents}
 * <p/>
 * 自身状态变化通知{@link RoundsConsultRoomActivity.InnerRoomSelfStateListener}
 * <p/>
 * 其他成员状态变化通知{@link RoundsConsultRoomActivity.InnerRoomMemberStateListener}
 *
 * @author Yang²
 */
@SuppressWarnings("ALL")
public class RoundsConsultRoomActivity extends BaseActivity implements NetStateReceiver.NetworkStateChangedListener, MessageStateChangeListener, SmallVideoView.onSmallVideoClickListener, ToastLayoutView.OnToastPositiveClickListener {
            ...
            if (intent.getAction().equals(Intent.ACTION_HEADSET_PLUG)) {
                ...
                switch (state) {
                    case 0: //拔出耳机
                        ...
                        //  拔出耳机
                        mSpeakerIb.setImageDrawable(getCompatDrawable(R.drawable.ic_btn_video_room_speaker_on));
                    ...
                    case 1: //插耳机
                        ...
                        mSpeakerIb.setImageDrawable(getCompatDrawable(R.drawable.ic_btn_video_room_speaker_off));
    ...
    private void initData() {
        mSuperiorExpertId = getIntent().getIntExtra(ExtraDataKeyConfig.EXTRA_DATA_KEY_DOCTOR_ID, 0);
    ...
    private void initView() {
        ...
        mConsultListTv.setVisibility(View.GONE);
        mClinicTv.setVisibility(View.VISIBLE);
        mBigScreenSv = new GLSurfaceView(this);
        mRecordTv.setText(getString(R.string.rounds_room_details));
        ...
        request.appointmentId = mOrderId;
        request.doPost();
    }
    ...
    private void initListener() {
        ...
        mSelfStateChangeListener = new RoundsConsultRoomActivity.InnerRoomSelfStateListener();
        mConsultRoomManager.registerMemberChangeListener(new RoundsConsultRoomActivity.InnerRoomMemberStateListener());
    ...
    private void initPage() {
        ...
        OrderDetailsRequester requester = new OrderDetailsRequester(new DefaultResultCallback<OrderDetailsInfo>() {
            ...
            public void onSuccess(OrderDetailsInfo orderDetailsInfo, BaseResult baseResult) {
                ...
                mRoomId = orderDetailsInfo.getRoomId();
                getScheduleAppointIDs(0);
                ...
                initCallWindow();
                joinRoom(mRoomId);
                displayTitile(orderDetailsInfo);
            }
    ...
    private void displayTitile(final OrderDetailsInfo orderDetailsInfo) {
        mDoctorManager.getDoctor(orderDetailsInfo.getLaunchDoctorId(), new DoctorManager.OnDoctorInfoLoadListener() {
            @Override
            public void onSuccess(DoctorBaseInfo doctorBaseInfo) {
                if (doctorBaseInfo != null) {
                    mTitleHospitalName = doctorBaseInfo.getHospitalName();
                    mClinicTv.setText((mTitleHospitalName == null ? "" : mTitleHospitalName) + (mTitileDepartmentName == null ? "" : mTitileDepartmentName) + orderDetailsInfo.getOrderTitle());
                    mTitileDepartmentName = doctorBaseInfo.getDepartmentName();
                    mClinicTv.setText((mTitleHospitalName == null ? "" : mTitleHospitalName) + (mTitileDepartmentName == null ? "" : mTitileDepartmentName) + orderDetailsInfo.getOrderTitle());
                }
            }
    ...
    private void setOtherDoctorId(OrderDetailsInfo orderDetailsInfo) {
        mAttendingDoctorId = orderDetailsInfo.getLaunchDoctorId();
        mSuperiorDoctorId = orderDetailsInfo.getDoctorId();
    ...
    private void reqFinishAppoint() {
        showProgressDialog();
        FinishOrderVisitRequester requester = new FinishOrderVisitRequester(new OnResultListener<Void>() {
            @Override
        ...
        View.OnClickListener callListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                switch (v.getId()) {
                    case 0:
                        //一键呼叫
                        if (mOrderDetailsInfo.getRoomId() == mRoomId) {
    ...
    private void addMembersJoinVideo(int userType) {
        int userId;
        if (userType == AppConstant.UserType.USER_TYPE_CUSTOMER_SERVICE) {
            userId = mCustomerServiceId;
        } else {
            userId = mOrderDetailsInfo.getLaunchDoctorId();
        }
    ...
    private void callDoctor() {
        ...
        ConsultRoomUtils.sendCallMessage(myUserId, mAttendingDoctorId, mRoomId, AppConstant.UserType.USER_TYPE_ATTENDING_DOCTOR, 14);
...
//        setCurrentIndex();
    ...
    private void addSelfVideo() {
        ...
        RoundsConsultRoomActivity.MyVideoRendererGui rendererGui = new RoundsConsultRoomActivity.MyVideoRendererGui(glSurfaceView, this);
    ...
    private void initBigVideo(VideoInfo videoInfo, int videoType) {
        ...
        rl.setBackgroundColor(getCompatColor(R.color.color_2f2f2f));
...
//                lp.setMargins(left, top, left, top);
    ...
    private void joinVideoRoom(int roomId) {
        ...
        mConsultRoomManager.joinVideoRoom(mOrderId, roomId, mUserType, "", NetWorkUtil.getNetworkType(getActivity()), mSelfStateChangeListener);
    }
    ...
    private void switchAvModeView() {
        ...
        if (mCurrentAvType == ConsultRoomManager.AV_TYPE_VIDEO) {
            mAvModeIb.setImageDrawable(getCompatDrawable(R.drawable.ic_video_room_video_on));
            ...
            mAvModeIb.setImageDrawable(getCompatDrawable(R.drawable.ic_video_room_video_off));
    ...
    private void setMicState() {
        mMicIb.setImageDrawable(mIsMicOpen ? getCompatDrawable(R.drawable.ic_btn_video_room_mic_on) : getCompatDrawable(R.drawable.ic_btn_video_room_mic_off));
...
//        Logger.logI(Logger.ROOM, "InnerLMVideoEvents->getNetState->v" + v + "->v" + v + "->state:" + state);
    ...
    private void exitVideoRoom() {
        ...
        Logger.logD(Logger.ROOM, "exitVideoRoom（）->mAppointmentId:" + mOrderId + "->mSelfStateChangeListener:" + mSelfStateChangeListener);
        stopVideo();
        mConsultRoomManager.exitVideoRoom(mRoomId, mSelfStateChangeListener);
    }
    ...
    public void onNewMessage(final BaseMessageInfo baseMessageInfo) {
        ...
        if (baseMessageInfo.getMsgType() == MessageProtocol.SMS_TYPE_DOCTOR_CALL || baseMessageInfo.getMsgType() == MessageProtocol.SMS_TYPE_DOCTOR_ASSISTANT_CALL) {
            ...
            if (baseMessageInfo.getAppointmentId() != mOrderId) {
                OrderDetailsRequester requester = new OrderDetailsRequester(new DefaultResultCallback<OrderDetailsInfo>() {
                    ...
                    public void onSuccess(OrderDetailsInfo orderDetailsInfo, BaseResult baseResult) {
                        ...
                        if (orderDetailsInfo.getRoomId() == mRoomId) {
                            mUserType = userType;
                            mOrderId = orderDetailsInfo.getOrderId();
                            mOrderDetailsInfo = orderDetailsInfo;
                            setOtherDoctorId(mOrderDetailsInfo);
                            setCurrentIndex();
                            if (mFlagInRoom) {
                                return;
                            }
                            ...
                            showCallDialog(orderDetailsInfo, baseMessageInfo, baseMessageInfo.getSenderID(), userType);
                        }
                    }
                });
                requester.orderId = baseMessageInfo.getAppointmentId();
                requester.doPost();
            }
            ...
            Logger.logD(Logger.ROOM, "onNewMessage()->getAppointmentId:" + baseMessageInfo.getAppointmentId() + ",mAppointmentId:" + mOrderId);
            if (baseMessageInfo.getAppointmentId() != mOrderId) {
                return;
            }
            ...
            if (baseMessageInfo.getSenderID() == mCustomerServiceId) {
                mCustomerServiceCalling = false;
                mIsMeCall = false;
                ...
                setVideoState(mOrderDetailsInfo.getLaunchDoctorId(), AppConstant.RoomState.STATE_MISS);
                VideoInfo videoInfo = getVideoInfo(mOrderDetailsInfo.getLaunchDoctorId(), VideoTagInfo.VIDEO_TYPE_CAMERA);
                if (videoInfo != null) {
                    videoInfo.setVideoState(AppConstant.RoomState.STATE_MISS);
                }
    ...
    private void showConsultWaitDialog() {
        ...
        //mConsultWaitDialog.setConsultInfo(mOrderDetailsInfo);
    }
    ...
    private void showDoctorVideoReqDialog() {
        ...
        mDoctorReqDialog = new CommonDialog.Builder(this)
                ...
                .setPositiveButton(R.string.video_dialog_no, new CommonDialog.OnPositiveBtnClickListener() {
                    @Override
                    public void onPositiveBtnClicked() {
                        ConsultRoomUtils.respondVideoReq(myUserId, mAttendingDoctorId, mOrderId, ConsultRoomUtils.CALL_RESULT_REFUSE);
                ...
                .setNegativeButton(R.string.video_dialog_yes, new CommonDialog.OnNegativeBtnClickListener() {
                    @Override
                    public void onNegativeBtnClicked() {
                        ConsultRoomUtils.respondVideoReq(myUserId, mAttendingDoctorId, mOrderId, ConsultRoomUtils.CALL_RESULT_ALLOW);
                        ConsultRoomUtils.clearTimer(mTimers, TIMER_TYPE_RESPOND_DOCTOR);
                    }
                })
                .setCanceledOnTouchOutside(false)
     ...
     * 弹呼叫Dialog
     */
    private void showCallDialog(final OrderDetailsInfo orderDetailsInfo, BaseMessageInfo baseMessageInfo, final int doctorId, final int userType) {
        Logger.logD(Logger.ROOM, "onNewMessage()->showCallDialog:" + orderDetailsInfo.getRoomId() + ",mroonId:" + mRoomId);
        ...
        leftTv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mCallDialog.dismiss();
                ConsultRoomUtils.sendRefuseMsg(doctorId, orderDetailsInfo.getOrderId());
            }
        });
        ...
        mDoctorManager.getDoctor(baseMessageInfo.getSenderID(), new DoctorManager.OnDoctorInfoLoadListener() {
            ...
            public void onSuccess(DoctorBaseInfo doctorBaseInfo) {
                ...
                mCallDialogHospital = doctorBaseInfo.getHospitalName();
                mCallDialogDepartment = doctorBaseInfo.getDepartmentName();
...
//        mToastTlv.showToastView(getString(R.string.video_room_state_subscribe, getUserTypeTitle(videoInfo.getUserType())));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.activity_consult_room_help_core:
                HelpCoreActivity.start(RoundsConsultRoomActivity.this);
                break;
    ...
    private void getOrderDetails() {
        mDoctorManager.getDoctor(mOrderDetailsInfo.getLaunchDoctorId(), new DoctorManager.OnDoctorInfoLoadListener() {
            @Override
            public void onSuccess(DoctorBaseInfo doctorBaseInfo) {
                if (doctorBaseInfo != null) {
                    if (mIsRequeste) {
                        mIsRequeste = false;
                        Intent intent = new Intent(getActivity(), RoundsDetailActivity.class);
                        intent.putExtra(ExtraDataKeyConfig.EXTRA_DATA_KEY_ORDER_ID, mOrderDetailsInfo.getOrderId());
    ...
    private void seekHelp() {
        mDoctorManager.getDoctorInfo(mUserInfoManager.getCurrentUserInfo().getUserId(), false, new DoctorManager.OnDoctorInfoLoadListener() {
            @Override
            public void onSuccess(DoctorBaseInfo doctorBaseInfo) {
                Logger.logD(Logger.ROOM, "callServicePersonnel->name:" + mUserInfoManager.getCurrentUserInfo().getUserName() + "<userId:" + mUserInfoManager.getCurrentUserInfo().getUserId());
    ...
    private void callServicePersonnel(VideoRoomResultInfo roomResultInfo) {
        ...
        ConsultRoomUtils.sendCallustomerServiceMessage(myUserId, roomResultInfo.getUserID(), mOrderId, AppConstant.UserType.USER_TYPE_CUSTOMER_SERVICE, roomResultInfo.getSeqId());
...
//            Logger.logD(Logger.ROOM, "onVideoNetworkState->streamSsrc:" + streamSsrc + "->state:" + state);
        ...
        public void onSelfJoinRoom(int result, VideoRoomResultInfo resultInfo) {
            ...
            if (result == VideoRoomResultCode.VIDEO_ROOM_RESULT_CODE_SUCCESS) {//加入成功
                ...
                for (int i = 0; i < memberList.size(); i++) {
                    if (memberList.get(i).getUserType() == AppConstant.UserType.USER_TYPE_ATTENDING_DOCTOR
                ...
                //重新呼叫
                if (mCallActionType == CALL_ACTION_TYPE_DOCTOR) {
                    callDoctor();
                }
        ...
        public void onSelfExitRoom(int result, VideoRoomResultInfo resultInfo) {
            ...
            if (result == VideoRoomResultCode.VIDEO_ROOM_RESULT_CODE_SUCCESS) {
                ...
                if (mCallActionType == CALL_ACTION_TYPE_DEFAULT) {
                    ...
                    resetState();
                    dismissProgressDialog();
                    joinRoom(mOrderDetailsInfo.getRoomId());
                }
            } else {
                dismissProgressDialog();
    ...
    private void showMemberListDialog(List<VideoRoomMember> memberList) {
        ...
        if (memberList.size() == 0) {
            ...
            MemberListDialogAdapter adapter = new MemberListDialogAdapter(R.layout.item_live_video_member_list, new ArrayList<>(0), mOrderId);
...
//            Logger.logD(Logger.ROOM, TAG + "->onMemberSpeaking()->userId:" + roomResultInfo.getUserID() + ", seatState:" + roomResultInfo.getSeatSate());
    ...
    private static class MyVideoRendererGui extends VideoRendererGui {
        ...
        private MyVideoRendererGui.OnCutBitmapListener mOnCutBitmapListener;

        public MyVideoRendererGui(GLSurfaceView surface, Context context) {
            super(surface, context);
        }

        public void setCutBitmap(int width, int height, MyVideoRendererGui.OnCutBitmapListener listener) {
            mWidth = width;
            mHeight = height;
            mIsCutBitmap = true;
