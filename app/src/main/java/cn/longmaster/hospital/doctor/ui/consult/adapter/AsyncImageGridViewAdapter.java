...
public class AsyncImageGridViewAdapter extends BaseAdapter {
    ...
    private List<AuxiliaryMaterialInfo> mData;
    private int mLength;
    private MediaDownloadManager mMediaDownloadManager;
    private ScrollGridView mGridView;
    private boolean mShowName;

    public AsyncImageGridViewAdapter(Context context, List<AuxiliaryMaterialInfo> patient_auxiliary_check_list) {
        mContext = context;
        mShowName = true;
        mData = new ArrayList<>();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.initState();
        final AuxiliaryMaterialInfo info = mData.get(position);
        if (info.getMaterialType() == AppConstant.MaterailType.MATERAIL_TYPE_PIC) {
            try {
                ...
                //String filePath = SdManager.getInstance().getOrderPicPath(smallPicName, info.getAppointmentId() + "");
            ...
            if (info.getMediaType() == AppConstant.MediaType.MEDIA_TYPE_VIDEO) {
                ...
                if (info.getMediaDownloadInfo() == null) {
                    if (FileUtil.isFileExist(mMediaDownloadManager.getFilePath(info.getMaterialPic(), info.getAppointmentId()))) {
                        info.setMediaDownloadInfo(new MediaDownloadInfo(info.getAppointmentId(), DownloadState.DOWNLOAD_SUCCESS, info.getMaterialPic()));
                        ...
                        mMediaDownloadManager.getDownloadInfoFromDB(info.getMaterialPic(), new MediaDownloadManager.GetMediaDownloadCallback() {
                            ...
                            public void onGetMediaDownload(MediaDownloadInfo mediaDownloadInfo) {
                                if (mediaDownloadInfo != null) {
                                    if (!FileUtil.isFileExist(mMediaDownloadManager.getTempPath(info.getMaterialPic(), info.getAppointmentId()))) {
                                    ...
                                    mediaDownloadInfo = new MediaDownloadInfo(info.getAppointmentId(),
            ...
            } else if (info.getMediaType() == AppConstant.MediaType.MEDIA_TYPE_VOICE) {
