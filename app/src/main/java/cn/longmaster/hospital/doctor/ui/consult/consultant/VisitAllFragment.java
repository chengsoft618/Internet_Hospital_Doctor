...
public class VisitAllFragment extends NewBaseFragment {
    ...
    private void displaySearchData() {
        ...
        Logger.logI(Logger.COMMON, "VisitAllFragment->onStart()-->searchScheduingInfo:" + mSearchScheduingInfo + ",visitScreenInfo:" + mVisitScreenInfo);
        displayData();
    }
    ...
    private void initData() {
        mScheduingInfo = (ScheduingListInfo) getArguments().getSerializable(ExtraDataKeyConfig.EXTRA_DATA_KEY_SCHEDULE_LIST);
    }
    ...
    private void initView() {
        getSchedulePayment(mScheduingInfo);
        getScheduleService();
        initPullRefreshView();
        mAppointmentIdTv.setText(getString(R.string.consult_payment_confirm_schedule_id, mScheduingInfo.getScheduingId() + ""));
        String str = getString(R.string.advance_d);

        for (int i = 0; i < mScheduingInfo.getAttDoctorList().size(); i++) {
            if (i == mScheduingInfo.getAttDoctorList().size() - 1 || i == 3) {
                str += mScheduingInfo.getAttDoctorList().get(i).getAttDoctorName();
                break;
            } else {
                str += mScheduingInfo.getAttDoctorList().get(i).getAttDoctorName() + ",";
            }
    ...
    private void getScheduleService() {
        ScheduleServiceRequester serviceRequester = new ScheduleServiceRequester((baseResult, scheduleOrImageInfo) -> {
            Logger.logI(Logger.COMMON, "VisitAllFragment-->getScheduleService-->scheduleOrImageInfo:" + scheduleOrImageInfo);
        ...
        serviceRequester.scheduingId = mScheduingInfo.getScheduingId();
    ...
    private void getVisitList(String beginDt, String endDt, int doctorId, String keyWord, String searchType) {
        GetVisitListRequester requester = new GetVisitListRequester((baseResult, visitInfo) -> {
            Logger.logI(Logger.COMMON, "VisitAllFragment-->GetVisitListRequester-->visitInfo:" + visitInfo);
            setVisitInfo(visitInfo, baseResult);
        });
        requester.statNum = AppConstant.ConsultantStatusType.APPOINTMENT_STATUS_TYPE_ALL;
        requester.scheduingId = mScheduingInfo.getScheduingId();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.fragment_visit_all_confirmation_sheet:
                ...
                intent.putExtra(ExtraDataKeyConfig.EXTRA_DATA_KEY_SCHEDULE_ID, mScheduingInfo.getScheduingId());
                startActivity(intent);
                break;
            default:
                break;
